...
namespace CustomGraphics
{
    public class Dashed
    {
        ...
        public static void Draw(LTGLineRenderer lineRenderer, Line line)
        {
            ...
            if (linePointsCount > 0)
            {
                ...
                for (float f = dashSize; f <= line.length; f += dashSize)
                {
                    ...
                    float w0 = LTGUtility.ConvertScale(lastF, 0, line.length, sWidth, eWidth);
                    float w1 = LTGUtility.ConvertScale(f, 0, line.length, sWidth, eWidth);
