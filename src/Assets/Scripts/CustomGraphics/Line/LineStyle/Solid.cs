...
namespace CustomGraphics
{
    public class Solid
    {
        ...
        public static new void Draw(LTGLineRenderer lineRenderer, Line line)
        {
            ...
            if (linePointsCount > 0)
            {
                ...
                for (int i = 1; i < linePointsCount; i++)
                {
                    ...
                    float w0 = LTGUtility.ConvertScale(currentLenght - lengthToAdd, 0, line.length, sWidth, eWidth);
                    float w1 = LTGUtility.ConvertScale(currentLenght, 0, line.length, sWidth, eWidth);
                    ...
                    if (lastAngle != angle && i > 1 && i < linePointsCount)
                    {
                        ...
                        lineRenderer.AddUIVertexQuad(new[] { pos0_, pos1_, pos2_, pos3_ }, lineRenderer.UVs,
                            line.color);
                    }
