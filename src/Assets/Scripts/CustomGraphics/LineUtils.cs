...
namespace CustomGraphics
{
    public class LineUtils
    {
        ...
        public static Vector2[] ConvertLinePointsToCurve(Vector3[] linePoints, Connection.CurveStyleType curveStyle)
        {
            switch (curveStyle)
            {
                case Connection.CurveStyleType.Spline:
                    return LineUtils.GetSPLineCurve(
                        linePoints[0],
                        linePoints[1],
                        linePoints[2],
                        linePoints[3]);
                case Connection.CurveStyleType.Z_Shape:
                    return new Vector2[] {
                        linePoints[0],
                        (linePoints[1] + linePoints[0])/2,
                        (linePoints[2] + linePoints[3])/2,
                        linePoints[3] };
                case Connection.CurveStyleType.Soft_Z_Shape:
                    return LineUtils.GetSoftZShape(
                        linePoints[0],
                        (linePoints[1] + linePoints[0]) / 2,
                        (linePoints[2] + linePoints[3]) / 2,
                        linePoints[3]);
                case Connection.CurveStyleType.Line:
                    return new Vector2[] {
                        linePoints[0],
                        linePoints[3] };
                default:
