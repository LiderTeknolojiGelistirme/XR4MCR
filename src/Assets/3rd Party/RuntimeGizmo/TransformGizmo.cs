...
namespace RuntimeGizmos
{
    ...
    {
        XRInputManager _xrInputManager;
        ...
        public float moveSpeedMultiplier = .003f;
        public float scaleSpeedMultiplier = .003f;
        public float rotateSpeedMultiplier = .003f;
        public float allRotateSpeedMultiplier = 20f;

        public bool useFirstSelectedAsMain = true;
        public bool isTriggerPressed = false;
        public bool isTriggerReleased = true;
        ...
        public Transform mainTargetRoot
        {
            get
            {
                return (targetRootsOrdered.Count > 0)
                    ? (useFirstSelectedAsMain)
                        ? targetRootsOrdered[0]
        ...
        {
            SetMaterial();
            _xrInputManager = FindObjectOfType<XRInputManager>();
        ...
        void TransformSelected()
        {
            ...
            {
                if (nearAxis != Axis.None && isTriggerPressed)
                {
        ...
        IEnumerator TransformSelected(TransformType transType)
        {
            ...
            while (!isTriggerReleased)
            {
                Vector3 origin, direction;
                _xrInputManager.xrRayInteractor.GetLineOriginAndDirection(out origin, out direction);
                Vector3 controllerPosition = Geometry.LinePlaneIntersect(origin, direction, originalPivot, planeNormal);

                // XR girdisiyle snapping kontrolü
                bool isSnapping = _xrInputManager.IsSnapPressed();

                if (previousControllerPosition != Vector3.zero && controllerPosition != Vector3.zero)
                {
                    if (transType == TransformType.Move)
                    {
                        ...
                        {
                            movement = controllerPosition - previousControllerPosition;
                        }
                        else
                        {
                            float moveAmount =
                                ExtVector3.MagnitudeInDirection(controllerPosition - previousControllerPosition,
                                    projectedAxis) * moveSpeedMultiplier;
                        ...
                        if (isSnapping && movementSnap > 0)
                        {
                            ...
                            {
                                float remainder;
                                float snapAmount = CalculateSnapAmount(movementSnap,
                                    currentSnapMovementAmount.magnitude, out remainder);
                    ...
                    else if (transType == TransformType.Scale)
                    {
                        ...
                        float scaleAmount =
                            ExtVector3.MagnitudeInDirection(controllerPosition - previousControllerPosition,
                                projected) * scaleSpeedMultiplier;
                        ...
                        // Uyarı: Unity 5.4/5.5'teki bir bug nedeniyle InverseTransformDirection scale'dan etkileniyor.
                    ...
                    else if (transType == TransformType.Rotate)
                    {
                        ...
                        // Raycast ile seçilen rotation gizmosu üzerinden controller'ın pozisyon farkından açısal fark hesaplanıyor.
                        float angle = Vector3.SignedAngle(previousControllerPosition - originalPivot,
                            controllerPosition - originalPivot, axis);
                        ...
                        if (isSnapping && rotationSnap > 0)
                        {
                            ...
                            float remainder;
                            float snapAmount =
                                CalculateSnapAmount(rotationSnap, currentSnapRotationAmount, out remainder);
                            if (snapAmount != 0)
                            {
            ...
            // İşlemler tamamlandığında transform değerlerini kaydediyoruz.
            ...
            SetTranslatingAxis(transType, Axis.None);
            SetPivotPoint();
        }
        ...
        void GetTarget()
        {
            if (nearAxis == Axis.None && isTriggerPressed)
            {
                ...
                RaycastHit hitInfo;
                if (_xrInputManager.xrRayInteractor.TryGetCurrent3DRaycastHit(out hitInfo))
                {
                    Transform target;
                    if (hitInfo.transform.CompareTag("UR10"))
                    {
                        target = hitInfo.transform;
                    }
                    else if (hitInfo.transform.CompareTag("sphere"))
                    {
                        target = hitInfo.transform;
                    }
                    else
        ...
        {
            float xClosestDistance = ClosestDistanceFromControllerToLines(axisVectors.x);
            float yClosestDistance = ClosestDistanceFromControllerToLines(axisVectors.y);
            float zClosestDistance = ClosestDistanceFromControllerToLines(axisVectors.z);
            float allClosestDistance = ClosestDistanceFromControllerToLines(axisVectors.all);
        ...
        {
            float xClosestDistance = ClosestDistanceFromControllerToPlanes(axisVectors.x);
            float yClosestDistance = ClosestDistanceFromControllerToPlanes(axisVectors.y);
            float zClosestDistance = ClosestDistanceFromControllerToPlanes(axisVectors.z);
            float allClosestDistance = ClosestDistanceFromControllerToPlanes(axisVectors.all);
            ...
            float allClosestDistance, float minSelectedDistanceCheck)
        {
            ...
            else if (type == TransformType.Rotate && mainTargetRoot != null)
            {
                Vector3 origin, direction;
                _xrInputManager.xrRayInteractor.GetLineOriginAndDirection(out origin, out direction);
                Ray controllerRay = new Ray(origin, direction);
                Vector3 controllerPlaneHit = Geometry.LinePlaneIntersect(controllerRay.origin, controllerRay.direction,
                    pivotPoint,
                    (transform.position - pivotPoint).normalized);
                if ((pivotPoint - controllerPlaneHit).sqrMagnitude <= (GetHandleLength(TransformType.Rotate)).Squared())
                    SetTranslatingAxis(type, Axis.Any);
            }
        }

        float ClosestDistanceFromControllerToLines(List<Vector3> lines)
        {
            Vector3 origin, direction;
            _xrInputManager.xrRayInteractor.GetLineOriginAndDirection(out origin, out direction);
            ...
            for (int i = 0; i + 1 < lines.Count; i++)
            {
                IntersectPoints points =
                    Geometry.ClosestPointsOnSegmentToLine(lines[i], lines[i + 1], origin, direction);
        ...
        float ClosestDistanceFromControllerToPlanes(List<Vector3> planePoints)
        {
            float closestDistance = float.MaxValue;
            ...
            if (planePoints.Count >= 4)
            {
                ...
                for (int i = 0; i < planePoints.Count; i += 4)
                {
                    ...
                    if (plane.Raycast(controllerRay, out distanceToPlane))
                    {
                        Vector3 pointOnPlane = controllerRay.origin + (controllerRay.direction * distanceToPlane);
